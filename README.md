...
The collection has three top-level scopes:
...
The `90 Manual Requests` scope contains `10 Cache Lookup`.
...
The collection does not use authentication or cookies. Every request disables cookie send and cookie storage.
...
To run Full on a gateway, select only `00 Gateway Contract`. To run Full on Local or a direct VM, select `00 Gateway Contract` and `10 Direct Server Only`. Never include `90 Manual Requests` in an automatic run.
...
For Full, the runner sends only `00 Gateway Contract` to gateways. It sends `00 Gateway Contract` and `10 Direct Server Only` to Local and VMs. It never sends `90 Manual Requests`.
